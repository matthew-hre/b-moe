...
export const EnvSchema = z
  .object({
    ...
    PI_COMMAND: optionalNonEmptyString,
    PI_PROVIDER: optionalNonEmptyString,
    PI_MODEL: optionalNonEmptyString,
    PI_API_KEY: optionalNonEmptyString,
    PI_THINKING_LEVEL: optionalNonEmptyString,
    PI_TOOLS: optionalNonEmptyString,
    REDIS_HOST: z.string().min(1),
    REDIS_PORT: z.coerce.number().int().positive().optional(),
    DOCKER_HOST: z.string().min(1).default("local"),
    SANDBOX_IMAGE: optionalNonEmptyString,
  ...
  .transform((env) => ({
    ...
    piApiKey: env.PI_API_KEY,
    piThinkingLevel: env.PI_THINKING_LEVEL,
    piTools: env.PI_TOOLS,
    redisHost: env.REDIS_HOST,
    redisPort: env.REDIS_PORT ?? 6379,
    dockerHost: env.DOCKER_HOST,
    sandboxImage: env.SANDBOX_IMAGE ?? "b-moe/sandbox:latest",
